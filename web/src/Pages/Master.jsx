...
class Master extends Component {
  ...
  createLineChartData(data) {
    ...
    for (let i = 0; i < this.types.length; i++) {
      if (this.types[i] !== "Master") {
        returnData.push(
          this.createLineChartDataByType(this.types[i], data[this.types[i]])
        );
      }
    }
  ...
  getStats() {
    const stats = { hr: 0, goof: 0, yike: 0, dumb: 0, mistake: 0 };
    for (let type of this.state.totalChartData) {
      let max = this.getMaximum(Object.values(type));
      if (max == 0) {
        max = 1;
      }
      const value = type[this.state.selectedUser];
      const stat = Math.round((value / max) * 5);
      switch (type.id) {
        case "Dumb":
          stats.dumb = stat;
          break;
    ...
    return (
      <div>
        <Switch
          ...
          onSwitch={() => this._handleSwitchView()}
          option1="Graphs"
          option2="Card"
        />
      </div>
